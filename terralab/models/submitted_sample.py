...
class SubmittedSample(models.Model):
    ...
    name = fields.Char(translate=True, track_visibility='onchange') # Sample name
    ...
    target_use_ranges = fields.Many2many('terralab.targetuserange', track_visibility='onchange') # Attached Target Use Ranges (optional)
    ...
    def create(self, values):
        # Autonumber submitted samples per order
        sample_num = (self.env['terralab.submittedsample'].search_count([('order', '=', values['order'])]) or 0) + 1 if 'order' in values else 1
        logger.info('ORDER %s SAMPLE NUMBER %s' % (values.get('order', None), sample_num))
        values['num'] = sample_num
        # Default receive_date to submitted sample creation time
        if not values.get('receive_date'):
            values['receive_date'] = fields.Datetime.now()
        logger.debug('CREATE SUBMITTED SAMPLE %s' % (values))
        submitted_sample = super(SubmittedSample, self).create(values)
    ...
    def _message_log(self, *args, **kwargs):
        ret = super(SubmittedSample, self)._message_log(*args, **kwargs)
        logger.info('MESSAGE LOG SAMPLE %s %s %s' % (ret, args, kwargs))
